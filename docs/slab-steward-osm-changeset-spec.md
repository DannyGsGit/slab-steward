...
## 9. Organised Editing compliance

Steward is coordinated organisational editing. You're in scope. The obligations are cheap; skipping them is expensive.

**Create `Organised Editing/Activities/SLAB Steward` on the OSM wiki, containing:**

- coordinating person or organisation, and a contact method
- **the unique hashtag** used in changeset comments (`#slabsteward`)
- the goal, and *why* it's being pursued
- timeframe
- non-standard tools and data sources, their usage conditions, and links where the community can access them
- participating accounts (for those who want to be identified)
- any performance metrics you measure participants against
- training materials and written instructions given to participants
- your plan for post-activity validation and cleanup
- results, on completion or monthly for ongoing work

**Also:**

- Notify affected communities **at least two weeks before launch**, on channels that are open (no non-OSM registration), public, and archived — the OSM US Slack `#trails`, the community forum, and the relevant regional channel.
- Make sure contributors understand community consensus and mapping practice before they edit.
- Link the activity page from participant profiles.
- Keep all related discussion on public channels, not DMs.

Enforcement is lighter than the text implies — the DWG has said it will intervene over edits the community objects to, not over guideline non-compliance alone. But compliance costs roughly a day and buys you the presumption of good faith, which is the only currency that matters when the first complaint arrives.

### Where the mechanical-edit line sits

The Automated Edits code of conduct covers changes made *"without review individually by the person controlling the edits"* — and it explicitly includes *"manually changing tags without adequate review."* Bulk operations through a standard editor still count if objects aren't individually evaluated.

So:

- **Not mechanical:** a user reviews each trail and confirms a value. Your P0 constraint ("never write to OSM without a human confirming") keeps you here.
- **Mechanical:** "set `operator=WMBC` on all 203 trails in this polygon" applied without per-object review. Even with a human clicking the button.

The bulk attribute editor lives near this line by design. Draw it explicitly in the UI: an operation that applies one value to a selection *without the user seeing each object* needs the separate documentation path — a wiki page under `Automated edits/`, prior community discussion, `mechanical=yes` on the changeset, and a `description` linking the documentation. Build the preview screen so that bulk operations force per-object confirmation, and you stay on the right side of it.
...
- [ ] OAuth 2.0 app registered (PKCE, `write_api` + `read_prefs`), production and dev
- [ ] All write testing against `api06.dev.openstreetmap.org`
- [ ] `Organised Editing/Activities/SLAB Steward` wiki page live
- [ ] Two-week community notice posted (forum, OSM US Slack `#trails`, regional channel)
- [ ] Provenance gate live and refusing Trailforks-derived data by default
- [ ] Revert plan documented and rehearsed on dev

**Per submission**

- [ ] Re-`GET` every target element immediately before building the diff — no cached bodies
- [ ] Version echoed from that same read
- [ ] Complete `<nd>` list copied verbatim, order preserved
- [ ] All pre-existing tags reproduced; only the target key changed or added
- [ ] `check_date:<key>` set for the attribute touched
- [ ] Changeset tags set: `created_by`, `comment`, `hashtags`, `source`, `host`, `locale`, `slab:*`
- [ ] Comment shown to the user and editable before submit
- [ ] Diff upload used (atomic), not per-element PUT
- [ ] ≤10,000 elements; geographically coherent bounding box
- [ ] Changeset closed explicitly
- [ ] 409 auto-merge attempted only when the conflicting edit didn't touch the same key
- [ ] 429 backoff with jitter; first-week accounts throttled proactively

**Ongoing**

- [ ] Poll changeset discussions on Steward-created changesets and surface them to the user — an unanswered changeset comment is how a routine question becomes a DWG ticket
- [ ] Monthly results update on the OEG wiki page
- [ ] Bump `created_by` version on every release

---

## 11. Reference implementation sketch

Using `osm-api` (osmlab/osm-api-js, ~24 kB, converts OSM XML to JSON automatically):

```js
import * as OSM from "osm-api";

await OSM.login({ mode: "popup", clientId: CLIENT_ID, redirectUrl: LAND_URL });

/**
 * Apply a single tag change to existing ways. Geometry untouched.
 * Every element is re-read here, at submit time, on purpose.
 */
async function applyAttribute({ wayIds, key, value, region, org, source }) {
  // ② read fresh — never reuse a body fetched when the form was opened
  const ways = await OSM.getFeatures("way", wayIds);

  const modified = ways.map((w) => ({
    ...w,                                  // preserves nodes[] and version
    tags: {
      ...w.tags,                           // every pre-existing tag survives
      [key]: value,
      [`check_date:${key}`]: today(),
    },
  }));

  // ③ + ④ + ⑤ — library opens, uploads atomically, and closes
  return OSM.uploadChangeset(
    {
      created_by: `Slab Steward ${VERSION}`,
      comment: `Set ${key} on ${wayIds.length} trails at ${region} from ${source} #slabsteward`,
      ...
      "slab:workflow": "bulk-attribute",
      "slab:attribute": key,
      "slab:provenance": source,
    },
    { create: [], modify: modified, delete: [] }
  );
}
```

The spread of `...w` is doing the load-bearing work: it carries the node list and version straight through from the read. **Do not construct the way object field by field** — that's how a node list goes missing.

`uploadChangeset` takes `(tags, diff, options?)` and chunks automatically across the 10,000-element ceiling, exposing `onChunk` / `onProgress` hooks. Note that chunking means multiple changesets, so a very large selection loses the single-transaction guarantee across chunk boundaries — another reason to keep operations region-scoped.

Wrap the call in a 409 handler that re-reads, checks whether the conflicting edit touched `key`, and retries at most twice.
